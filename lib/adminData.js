...
export async function getPendingQueue() {
  ...
  const [opportunities, events, announcements] = await Promise.all([
    supabase.from('opportunities').select(`*, ${submitter}`).in('status', ['pending', 'resubmitted']).order('created_at', { ascending: true }),
    supabase.from('events').select(`*, ${submitter}`).in('status', ['pending', 'resubmitted']).order('created_at', { ascending: true }),
    supabase.from('announcements').select(`*, ${submitter}`).in('status', ['pending', 'resubmitted']).order('created_at', { ascending: true }),
  ]);
