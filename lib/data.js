...
export async function getEvents(major) {
  if (isDemoMode) {
    return filterByMajor(dedupeEvents([...fall2026AcademicEvents, ...sampleEvents]), major)
  ...
  if (major && major !== 'All majors') query = query.contains('majors', [major]);
  const { data, error } = await query;
  return filterByMajor(dedupeEvents([...fall2026AcademicEvents, ...requireData(data, error, 'events')]), major)
    .sort((a, b) => a.date.localeCompare(b.date));
}
