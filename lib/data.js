...
// All read functions below try Supabase first, and fall back to sample
// data if Supabase isn't configured yet (demo mode) or a query fails.
// This is what lets the app be fully demoable before setup is finished,
// and switches to real data automatically once Supabase env vars are set.
...
export async function getOpportunities(major) {
  ...
  if (error || !data) return filterByMajor(sampleOpportunities, major);
  return data;
}
...
export async function getEvents(major) {
  ...
  if (major && major !== 'All majors') query = query.contains('majors', [major]);
  const { data, error } = await query;
  if (error || !data) return filterByMajor(sampleEvents, major);
  return data;
}
...
export async function getAnnouncements() {
  ...
  const { data, error } = await supabase
    ...
    .eq('status', 'published')
    .order('created_at', { ascending: false });
  if (error || !data) return sampleAnnouncements;
  return data;
}
