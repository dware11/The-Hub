...
const checks = [
  ...
  ['super-admin links are gated', desktop.includes('{superAdmin &&') && desktop.includes('/admin/history') && desktop.includes('/admin/system')],
  ...
  ['history supports all requested filters', ['Action', 'Actor/user ID', 'Affected record', 'From date', 'To date', 'Sort'].every((label) => history.includes(`>${label}<`))],
